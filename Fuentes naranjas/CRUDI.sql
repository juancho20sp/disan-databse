...
CREATE OR REPLACE PACKAGE BODY PKG_PROCEDURE AS
    ...
    BEGIN 
        INSERT INTO Procedures VALUES (NULL, xName, xDate, NULL, NULL, xIdClinicalHistory, xIdHospital, NULL);
        COMMIT;
    ...
    BEGIN
        ...
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL INSERTAR EL DOCTOR EN EL PROCEDIMIENTO');  
    END;
    ...
    BEGIN
        INSERT INTO ProcedureNurse VALUES(xDocType, xDocNum, xIdProcedure);
        COMMIT;

        EXCEPTION 
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL INSERTAR LA ENFERMERA EN EL PROCEDIMIENTO');  
    END;
    ...
    BEGIN
        ...
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL FINALIZAR EL PROCEDIMIENTO');
    END;
...
CREATE OR REPLACE PACKAGE BODY PKG_APPOINTMENT AS
    ...
    BEGIN
        ...
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL INSERTAR EL PLAN DE MANEJO EN LA CITA MÉDICA');
    END;
...
-- PKG_MEDICINES
CREATE OR REPLACE PACKAGE BODY PKG_MEDICINES AS
    -- MEDICATION TYPE
    PROCEDURE ADD_MED_TYPE(
        xName IN VARCHAR
        ) IS
    BEGIN 
        INSERT INTO MedicationType VALUES(NULL, xName);  
        COMMIT;

        EXCEPTION 
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL INSERTAR EL TIPO DE MEDICAMENTO');
    END;

    -- READ ALL MED TYPES
    FUNCTION READ_MED_TYPES RETURN SYS_REFCURSOR
    IS INF_MED_TYPES SYS_REFCURSOR;
    BEGIN
        OPEN INF_MED_TYPES FOR
            SELECT idMedicationType AS ID,
            name AS NAME
            FROM MedicationType
            ORDER BY ID;
        RETURN INF_MED_TYPES ;
    END;
    ...
    BEGIN 
        INSERT INTO MedicationComponents VALUES(NULL, xName, NULL);  
        COMMIT;

        EXCEPTION 
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20001,'ERROR AL INSERTAR EL COMPONENTE DE MEDICAMENTO');
    END;

    -- READ ALL MED COMPONENTS
    FUNCTION READ_MED_COMPONENTS RETURN SYS_REFCURSOR
    IS INF_MED_COMP SYS_REFCURSOR;
    BEGIN
        OPEN INF_MED_COMP FOR
            SELECT ID_MED_COMPONENT,
